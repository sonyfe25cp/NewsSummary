...
public class MultipleCoreAMLMain {
	static final Logger logger = LoggerFactory.getLogger(MultipleCoreAMLMain.class);

	public static void main(String[] args) {
		MultipleCoreAMLMain amm = new MultipleCoreAMLMain();
		amm.runAll();
	}
	private void runAll(){
		List<String> events = sentenceService.findEvents();
		Vector<Evaluation> evas = new Vector<>();
		int errorCount = 0;
		...
		for(String event : events){
			executor.submit(new Worker(event, evas));
		}
		executor.shutdown();
		try {
...
//			}
			...
			Evaluation eva =  Evaluation.evaluateSummary(labeledSentence, summaryThisEvent);
			if(eva != null){
				evas.add(eva);
			}
		}
